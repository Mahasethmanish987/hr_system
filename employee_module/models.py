...
class Employee(TimeStamp):
    ...
    def save(self, *args, **kwargs):
        if not self.employee_code:
            # 1. Get department code (first 3 letters of department name)
            dept_code = self.department.name[:3].upper() if self.department else "GEN"

            # 2. Format date
            join_date = self.date_of_joining.strftime("%Y%m")

            # 3. Count how many employees already joined in that dept/month
            existing_count = (
...
# class TaskCompletion(TimeStamp):
...
#     def __str__(self):
#         return f"{self.task.title} - {self.employee.user.username}"
